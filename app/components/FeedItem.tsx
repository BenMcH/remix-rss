  ...
  return (
    <>
      <tr className="cursor-pointer border-t" onClick={() => setOpen(!open)}>
        <td className={`pl-2 ${open ? 'pt-2' : 'py-2'} md:truncate md:max-w-screen-2xl`}><span className="text-sm font-bold">{item.title}</span> {!open && <span className="text-xs font-light hidden md:inline">{contentSnippet}</span>}</td>
